...
async def open_accounts(call: CallbackQuery):
    ...
    await call.message.edit_text(t(lang, "accounts_title"), reply_markup=accounts_menu_kb(lang, accounts))
    await call.answer()
...
async def add_account_start(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.edit_text(t(lang, "send_phone"))
    await call.answer()
...
async def remove_account_menu(call: CallbackQuery):
    ...
    if not accounts:
        ...
        return
    await call.message.edit_text(t(lang, "choose_account_remove"), reply_markup=accounts_remove_kb(lang, accounts))
    await call.answer()
...
async def start_account_sniper(call: CallbackQuery):
    ...
    await call.message.edit_reply_markup(reply_markup=account_control_kb(lang, phone, True))
...
async def stop_account_sniper(call: CallbackQuery):
    ...
    await call.message.edit_reply_markup(reply_markup=account_control_kb(lang, phone, False))
